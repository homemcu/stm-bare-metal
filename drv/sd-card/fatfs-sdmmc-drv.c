...
static DRESULT read(uint8_t *rxbuf, uint32_t sector, uint32_t count)
{
	...
	for (cnt = 0; cnt < count; cnt++)
	{
		do
		{
			err = hal_sd_sdmmc_read_dma((uint8_t *)buf_dma, sector + cnt, 1);
		}
...
#else
	err = hal_sd_sdmmc_read(rxbuf, sector, count);
#endif
...
static DRESULT write(const uint8_t *txbuf, uint32_t sector, uint32_t count)
{
	...
	for (cnt = 0; cnt < count; cnt++)
	{
		...
		do
		{
			err = hal_sd_sdmmc_write_dma((const uint8_t *)buf_dma, sector + cnt, 1);
		}
		// wait for Transfer State
		while (err == sd_err_wrong_status);
		if (err != sd_err_ok)
...
#else
	err = hal_sd_sdmmc_write(txbuf, sector, count);
#endif
...
{
	return (DWORD)hal_sd_sdmmc_getcardinfo()->card_size;
}

//--------------------------------------------
static WORD getsectorsize(void)
{
	return (WORD)hal_sd_sdmmc_getcardinfo()->sect_size;
}

//--------------------------------------------
static DWORD getblocksize(void)
{
	return (DWORD)hal_sd_sdmmc_getcardinfo()->erase_size;
}
