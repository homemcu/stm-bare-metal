...
#define MAX_EP          4
#define MAX_RX_PACKET   512  /* in bytes */
#define MAX_CONTROL_EP  1
#define MAX_FIFO_SZ     320  /*in 32-bit chunks */

#define RX_FIFO_SZ      ((4 * MAX_CONTROL_EP + 6) + ((MAX_RX_PACKET / 4) + 1) + (MAX_EP * 2) + 1) /*in 32-bit chunks */
        ...
        if (_t & USB_OTG_DIEPCTL_USBAEP) {
            if (stall) {
                ...
                _BMD(_t, USB_OTG_DIEPCTL_STALL,
                     USB_OTG_DIEPCTL_SD0PID_SEVNFRM | USB_OTG_DIEPCTL_CNAK);
            }
            EPIN(ep)->DIEPCTL = _t;
        }
    } else {
...
/* The VBUS detection has not yet been made */
        ...
        *OTGPCTL = 0;
        ...
        OTG->GINTMSK  = USB_OTG_GINTMSK_USBRST | USB_OTG_GINTMSK_ENUMDNEM |
                        ...
                        USB_OTG_GINTMSK_PXFRM_IISOOXFRM | USB_OTG_GINTMSK_EOPFM |
...
static bool ep_config(uint8_t ep, uint8_t eptype, uint16_t epsize) {
    ...
    if (ep & 0x80) {
        ...
        case USB_EPTYPE_ISOCHRONOUS | USB_EPTYPE_DBLBUF:
            epi->DIEPCTL = (0x01 << 18) | USB_OTG_DIEPCTL_USBAEP |
        ...
        case USB_EPTYPE_ISOCHRONOUS:
        case USB_EPTYPE_ISOCHRONOUS | USB_EPTYPE_DBLBUF:
            epo->DOEPCTL = USB_OTG_DOEPCTL_CNAK |
...
static void evt_poll(usbd_device *dev, usbd_evt_callback callback) {
    ...
    while (1) {
        ...
        if (_t & USB_OTG_GINTSTS_USBRST) {
            ...
            evt = usbd_evt_wkup;
        } else if (_t & USB_OTG_GINTSTS_OEPINT) {
            for (;; ep++) {
                USB_OTG_OUTEndpointTypeDef* epi = EPOUT(ep);
                if (ep >= MAX_EP) break;
                if (epi->DOEPINT & USB_OTG_DOEPINT_XFRC) {
                    epi->DOEPINT = USB_OTG_DOEPINT_XFRC;
                    break;
                } else if (epi->DOEPINT & USB_OTG_DOEPINT_STUP) {
                    epi->DOEPINT = USB_OTG_DOEPINT_STUP;
                    break;
                } else if (epi->DOEPINT & USB_OTG_DOEPINT_OTEPDIS) {
                    epi->DOEPINT = USB_OTG_DOEPINT_OTEPDIS;
                    break;
                }
            }
            continue;
        } else if (_t & USB_OTG_GINTSTS_IISOIXFR) {
            for (;; ep++) {
                USB_OTG_INEndpointTypeDef* epi = EPIN(ep);
                if (ep >= MAX_EP) break;
                uint32_t diepctl = epi->DIEPCTL;
                if (((diepctl & USB_OTG_DIEPCTL_EPTYP) == USB_OTG_DIEPCTL_EPTYP_0) && 
                    ((diepctl & USB_OTG_DIEPCTL_EPENA) != 0)) {
                    epi->DIEPCTL |= USB_OTG_DIEPCTL_EPDIS;
                    break;
                }
            }
            ...
            return;
        } else if (_t & USB_OTG_GINTSTS_PXFR_INCOMPISOOUT) {
             OTG->GINTSTS = USB_OTG_GINTSTS_PXFR_INCOMPISOOUT;
             return;
        } else if (_t & USB_OTG_GINTSTS_EOPF) {
            for (;; ep++) {
                USB_OTG_INEndpointTypeDef* epi = EPIN(ep);
                USB_OTG_OUTEndpointTypeDef* epo = EPOUT(ep);
                if (ep >= MAX_EP) break;
                if ((epo->DOEPCTL & USB_OTG_DOEPCTL_EPTYP) == USB_OTG_DOEPCTL_EPTYP_0) {
                    /* isochronous OUT endpoint */
                    if (get_frame() & 1) {
                        _BST(epo->DOEPCTL, USB_OTG_DOEPCTL_SD0PID_SEVNFRM);
                    } else {
                        _BST(epo->DOEPCTL, USB_OTG_DOEPCTL_SODDFRM);
                    }
                } else if ((epi->DIEPCTL & USB_OTG_DIEPCTL_EPTYP) == USB_OTG_DIEPCTL_EPTYP_0) {
                    /* isochronous IN endpoint */
                    if (get_frame() & 1) {
                        _BST(epi->DIEPCTL, USB_OTG_DIEPCTL_SD0PID_SEVNFRM);
                    } else {
                         _BST(epi->DIEPCTL, USB_OTG_DIEPCTL_SODDFRM);
                    }
                }
            }
            OTG->GINTSTS = USB_OTG_GINTSTS_EOPF;
            return;
        } else {
