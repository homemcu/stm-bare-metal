...
void hw_cfg_pin(GPIO_TypeDef* gpioport, uint8_t pin, uint16_t gpiocfg)
{
	...
	((gpioport)->AFR[((pin)>>3)] = ((gpioport)->AFR[((pin)>>3)] & ~((0x000F)<<(((pin)&7)*4))) | ((gpiocfg & 0x000F)<<(((pin)&7)*4)));
	gpioport->MODER   = (gpioport->MODER   & ~(3 << pin2)) | (((gpiocfg >> 4  ) & 3) << pin2);
	gpioport->OSPEEDR = (gpioport->OSPEEDR & ~(3 << pin2)) | (((gpiocfg >> 7) & 3) << pin2);
	gpioport->OTYPER  = (gpioport->OTYPER  & ~(1 << pin )) | (((gpiocfg >> 6 ) & 1) << pin );
	gpioport->PUPDR   = (gpioport->PUPDR   & ~(3 << pin2)) | (((gpiocfg >> 9  ) & 3) << pin2);
}
...
void hw_cfg_extirq(uint8_t portidx, uint8_t pin, uint8_t irqcfg, uint8_t irqpr)
{
	...
	((RCC_TypeDef *) ((0x40000000UL + 0x00020000UL) + 0x3800UL))->APB2ENR |= (0x1UL << (14U));

	
	uint32_t tmp1 = (pin & 0x3) << 2;
	uint32_t tmp2 = ((uint32_t)0x0F) << tmp1;
	((SYSCFG_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x3800UL))->EXTICR[pin >> 2] = (((SYSCFG_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x3800UL))->EXTICR[pin >> 2] & ~tmp2) | (((uint32_t)portidx) << tmp1);

	
	uint32_t mask = (uint32_t)(1 << pin);
	((EXTI_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x3C00UL))->RTSR &= ~mask; 
	((EXTI_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x3C00UL))->FTSR &= ~mask; 
	switch (irqcfg & 0x38)
	{
		case 0x28:
	...
	((EXTI_TypeDef *) ((0x40000000UL + 0x00010000UL) + 0x3C00UL))->IMR |= mask;   

	
	uint8_t channel = (pin < 5) ? (EXTI0_IRQn + pin) : ((pin < 10) ? EXTI9_5_IRQn : EXTI15_10_IRQn);
	((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->IP[channel] = irqpr;
	((NVIC_Type *) ((0xE000E000UL) + 0x0100UL) )->ISER[channel >> 5] = 1 << (channel & 0x1F);  
}
